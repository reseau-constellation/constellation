...
import { cidValide, traduire, zipper } from "@/ipa/utils";


describe("Utils", function () {
  describe("cidValide", function () {
    step("valide", () => {
      const valide = cidValide("QmNR2n4zywCV61MeMLB6JwPueAPqheqpfiA4fLPMxouEmQ");
  ...
  describe("zipper", function () {
    ...
    before(async () => {
      ...
      const fichiersDocs = [{
        nom: "fichier1.txt", octets: Buffer.from("Je ne suis que du texte.")
      }]
      const fichiersSFIP = [
        {
          nom: "fichierSFIP1.txt", octets: Buffer.from("Je le fichier SFIP no. 1.")
        },
        {
          nom: "fichierSFIP2.txt", octets: Buffer.from("Je le fichier SFIP no. 2.")
        }
      ]
    ...
    step("Les documents de base existent", () => {
      ...
      const contenu = new TextDecoder().decode(fs.readFileSync(adresseFichier1));
    ...
    step("Les fichiers SFIP sont inclus",  () => {
      ...
      const adresseFichierSFIP1 = path.join(adresseFichiersSFIP, "fichierSFIP1.txt");
      expect(fs.existsSync(adresseFichierSFIP1)).to.be.true;
      const contenuSFIP1 = new TextDecoder().decode(fs.readFileSync(adresseFichierSFIP1));
      expect(contenuSFIP1).to.equal("Je le fichier SFIP no. 1.");

      const adresseFichierSFIP2 = path.join(adresseFichiersSFIP, "fichierSFIP2.txt");
      expect(fs.existsSync(adresseFichierSFIP2)).to.be.true;
      const contenuSFIP2 = new TextDecoder().decode(fs.readFileSync(adresseFichierSFIP2));
