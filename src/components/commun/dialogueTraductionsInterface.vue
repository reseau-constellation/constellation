        ...
        <v-row>
          <v-col cols="6">
            <v-list style="max-height: 500px" class="overflow-y-auto">
              <v-list-item
                v-for="clef in clefsMessages()"
                :key="clef"
                @click="() => sélectionner(clef)"
              >
                <v-list-item-content>
                  <v-card outlined max-width="300" class="mx-4">
                    <v-card-text class="text--primary">
                      {{
                        traduireClef(clef, [langueSource, langueOriginale]) ||
                        clef
                      }}
                    </v-card-text>
                    <v-divider />
          ...
          <v-col cols="6">
            <v-card outlined>
              ...
              <v-card-text>
                <v-card flat>
                  <v-textarea
                    :disabled="!clefSélectionnée"
                    outlined
                    no-resize
                    height="100"
                    :hint="clefSélectionnée"
                    persistent-hint
                  >
                  </v-textarea>
                  <v-card-actions>
                    <v-btn text :disabled="!clefSélectionnée"
                ...
                <v-card v-if="clefSélectionnée" flat>
                  <v-card-title>
                    Suggestions de la communauté ({{ 10 }})</v-card-title
                  >
                  <v-card-text>
                    <v-list style="max-height: 200px" class="overflow-y-auto">
                      <v-list-item
                        @click="codeÀNomLangue;"
                        v-for="i in 10"
                        :key="i"
                      >
                        <v-list-item-avatar>
                        ...
                        <v-list-item-content>
                          ...
                          <span class="text--secondary">{{
                            traduireClef(clefSélectionnée, [langueCible])
                          }}</span>
                        </v-list-item-content>
                      </v-list-item>
                    </v-list>
