...
const estUnDic = (x: unknown): boolean => typeof x === "object" && !Array.isArray(x)
const estUneListe = (x: unknown): boolean => Array.isArray(x)

const extraireBranches = (données: DonnéesJSON, clefs: clefsExtractionNonNul) => {
  let branches: élémentListe;

  if (!clefs.length) {
    branches = estUnDic(données) ? Object.values(données) : (données as élémentListe)
...
const injecterBranche = (base: élément, clefs: clefsExtractionNonNul, branche: valsJSON) => {
...
export const aplatirDonnées = (
  ...
  const { base, branches } = extraireBranches(données, clefsAvant as clefsExtractionNonNul);
  ...
  for (const branche of branches) {
    ...
    if (clefsAprès.length) {
      aplatirDonnées(
        branche, clefsAprès, copieBase, _élémentsFinaux
      );
    } else {
