...
console.log("Début du travailleur")
onmessage = async function ({ data }) {
  ...
  const extraireFonctionIPA = (fonction: string[]) => {
    ...
    const erreur = new Error(`Fonction ClientConstellation.${fonction.join(".")} n'existe pas.`)
    let fonctionIPA:
      | ClientConstellation
    ...
    for (const [i, attr] of fonction.entries()) {
      ...
      if (i === fonction.length - 1) {
        fonctionIPA = fonctionIPA[attr as keyof ClientConstellation].bind(fonctionIPA);
      } else {
