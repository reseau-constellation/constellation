...
interface Tâche {
  ...
  fOublier: utils.schémaFonctionOublier;
}
...
export class IPAParallèle extends Callable {
  ...
  async appelerFonctionSuivre(
    ...
    const f = listeArgs[iArgFonction] as utils.schémaFonctionSuivi<unknown>;
    ...
    await new Promise<void>((résoudre) => {
      once(this.événements, id).then(() => résoudre());
    });
  ...
  async appelerFonctionAction<T = unknown>(
    ...
    const promesse = new Promise<T>((résoudre) => {
      once(this.événements, id).then(résultat => résoudre(résultat[0]));
    });
